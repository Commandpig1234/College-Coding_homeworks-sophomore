...
typedef int Status;			//状态。函数返回值 OK(1) and ERROR(0)
typedef struct {
...
{
	L.elem = (ElemType *)malloc(LIST_MAX_SIZE*sizeof(ElemType));// 100 * int数组 0~99
//  if ( !L.elem )	exit(-1);	// 失败则终止程序 
  	L.length 	= 0;   			// 空表长度为0
  	L.listsize 	= LIST_MAX_SIZE;// LIST_MAX_SIZE == 100
  	return OK;
}
...
int main()
{	freopen("input/ZY2-4.txt","r",stdin);
...
Status GetElem(SqList L, int i, ElemType &e)//L中第i个给e
{
    if(i-1<=L.listsize){
        e = L.elem[i-1];
        return OK;
